      ...
      <table className="min-w-full">
        ...
        <tbody className="divide-y divide-gray-100">
          {entities.map((entity, index) => (
            ...
            >
              <td className="px-4 sm:px-6 py-4 sm:py-5 whitespace-nowrap text-sm font-medium text-gray-900">
                {entity[idField]}
              </td>
              {fields
                .filter((field) => !field.hidden)
                .map((field) => (
                  <td key={field.name} className="px-4 sm:px-6 py-4 sm:py-5 text-sm text-gray-700">
                    {field.type === "date"
                      ? new Date(entity[field.name]).toLocaleDateString("vi-VN")
                      : field.name === "gender"
                        ? entity[field.name] ? "Nam" : "Nữ"
                        : entity[field.name]}
                  </td>
                ))}
              {showStatusColumn && (
                <td className="px-4 sm:px-6 py-4 sm:py-5 text-sm text-gray-700">
                  <label className="inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={entity.user_status || false}
                      onChange={() => onToggleStatus(entity[idField], entity.user_status || false)}
