...
--cau3
CREATE OR REPLACE PROCEDURE THEM_GIANGVIEN_KIEMTRA_HAM
(
  P_MSGV IN VARCHAR2,
  P_TENGV IN VARCHAR2,
  P_SODT IN VARCHAR2,
...
IS  V_COUNT_MSGV NUMBER := 0;
    V_COUNT_MSHH NUMBER := 0;
BEGIN
  SELECT COUNT(*)
  INTO V_COUNT_MSGV
  FROM GIAOVIEN
  ...
  IF V_COUNT_MSGV >0 THEN 
    RAISE_APPLICATION_ERROR(-2002,'Lỗi: mã số giảng viên' || P_MSGV || ' đã tồn tại';
  ELSIF V_COUNT_MSHH > 0 THEN
    RAISE_APPLICATION_ERROR(-2001,'Lỗi: mã số học hàm' || P_MSHH || ' đã tồn tại';
  ELSE 
    INSERT INTO GIAOVIEN(MSGV, TENGV, SODT, DIACHI, MSHH, NAMHAM)
    VALUES (P_MSGV, P_TENGV, P_SODT, P_DIACHI, P_MSHH, P_NAMHAM);
    DBMS_OUTPUT.PUT_LINE('Đã thêm giảng viên ' || p_TENGV || ' thành công.');
  END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Lỗi: ' || SQLERRM);
END THEM_GIANGVIEN_KIEMTRA_HAM;

--cau4
CREATE OR REPLACE PROCEDURE CAPNHAT_TENDETAI (
    p_MSDT IN VARCHAR2,
    p_TENDETAI_MOI IN VARCHAR2
)
IS
    v_count NUMBER := 0;  
BEGIN
    ...
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20003, 'Lỗi: Mã số đề tài ' || p_MSDT || ' không tồn tại.');
    ELSE
        UPDATE DETAI
        SET TENDT = p_TENDETAI_MOI
        WHERE MSDT = p_MSDT;
        
        DBMS_OUTPUT.PUT_LINE('Đã cập nhật tên đề tài mới "' || p_TENDETAI_MOI || '" cho mã đề tài ' || p_MSDT || ' thành công.');
    END IF;

EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Lỗi: ' || SQLERRM);
END CAPNHAT_TENDETAI;
--cau5

CREATE OR REPLACE PROCEDURE CAPNHAT_SINHVIEN (
    p_MSSV IN VARCHAR2,
    p_TENSV_MOI IN VARCHAR2,
    p_DIACHI_MOI IN VARCHAR2
)
IS
    v_count NUMBER := 0;   
BEGIN
    ...
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20004, 'Lỗi: Mã số sinh viên ' || p_MSSV || ' không tồn tại.');
    ELSE
        UPDATE SINHVIEN
        SET TENSV = p_TENSV_MOI,
            DIACHI = p_DIACHI_MOI
        WHERE MSSV = p_MSSV;
        
        DBMS_OUTPUT.PUT_LINE('Đã cập nhật thông tin sinh viên ' || p_MSSV || ' thành công. Tên mới: ' || p_TENSV_MOI || ', Địa chỉ mới: ' || p_DIACHI_MOI);
    END IF;

EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Lỗi: ' || SQLERRM);
END CAPNHAT_SINHVIEN;
